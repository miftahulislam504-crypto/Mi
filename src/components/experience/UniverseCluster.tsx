...
// Small orbiting dot around universe cluster
function OrbitDot({
  radius,
  speed,
  offset,
...
export default function UniverseCluster({
  ...
  visible,
  index,
}: UniverseClusterProps) {
  const { phase, setActiveUniverse, setPhase, setCameraTarget } =
    useGalaxyStore();
  ...
  const scaleRef = useRef(0); // for entrance animation
  const opacityRef = useRef(0);

  const color = useMemo(() => new THREE.Color(universe.color), [universe.color]);

  // Orbit dots config — number matches planet count loosely
  ...
  // Entrance animation: stagger per index
  ...
  useFrame((_, delta) => {
    ...
    scaleRef.current = Math.min(1, t / 0.8); // 0.8s ease-in
    ...
    // Self-rotation (slow y-axis spin)
    if (coreRef.current) {
  ...
  return (
    ...
    >
      ...
      {/* Orbiting dots (represent planets) */}
      {visible &&
        orbitDots.map((dot, i) => (
          <OrbitDot
            key={i}
      ...
      {visible && (
        ...
        >
          ...
          {hovered && (
            ...
            >
              {universe.planets.length} planets · click to explore
            </div>
          )}
        </Html>
      )}
